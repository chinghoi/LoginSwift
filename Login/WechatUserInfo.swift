...
class WechatUserInfo: ViewController {
    ...
    override func viewDidLoad() {
        ...
        //网址可选值String转URL
        let url = URL(string: (wechatUser?.icon)!)
        //数据化网址
        ...
        lableUserName.text = "用户名:" + (wechatUser?.nickname)!
        ...
        let aboutMe = wechatUser?.aboutMe
        if aboutMe != nil {
            lableAboutMe.text = "用户简介:" + (wechatUser?.aboutMe)!
        } else {
            lableAboutMe.text = "用户简介: 无信息"
        }
        
        
        let birthday   = wechatUser?.birthday
